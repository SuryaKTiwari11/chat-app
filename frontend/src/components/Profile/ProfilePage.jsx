    ...
    const handleImageUpload = async (e) => {
        ...
        reader.onload = async() => {
            ...
            try {
                ...
                document.querySelectorAll('img[src="' + authUser.profilePic + '"]').forEach(img => {
                    img.src = profilePicWithTimestamp;
                });
                
                updateToast(toastId, "Profile picture updated successfully!", "success");
            } catch (error) {
                ...
                >
                            ...
                            <div className="relative w-32 h-32 rounded-full border-4 border-[#1E293B] overflow-hidden bg-[#1E293B] flex items-center justify-center shadow-lg group">
                                {selectedImg || authUser.profilePic ? (
                                    <img 
                                        ...
                                        className={`w-full h-full object-cover transition-all duration-500 ${selectedImg ? 'animate-pulse-once' : ''}`}
                                        onError={(e) => {
                                ...
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                    <div className={`${isUploadingImage ? 'flex' : 'hidden'} absolute inset-0 bg-black/70 items-center justify-center`}>
                                        <div className="w-8 h-8 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div>
                                    </div>
                                    <p className={`text-white text-sm font-medium ${isUploadingImage ? 'hidden' : 'block'}`}>Change Photo</p>
                                </div>
                            </div>
                            <label htmlFor="profile-image-upload" className={`absolute bottom-0 right-0 bg-gradient-to-br from-indigo-500 to-cyan-500 rounded-full p-2 cursor-pointer hover:shadow-lg hover:shadow-indigo-500/30 transition-all duration-300 transform hover:scale-110 ${isUploadingImage ? 'opacity-50 cursor-wait pointer-events-none' : ''}`}>
                    ...
                    <div className="pt-20 pb-8 px-8">
                        <div className="flex justify-between items-center mb-6">
                            <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">{authUser.fullname || "User"}</h1>
                            <button
                                type="button"
